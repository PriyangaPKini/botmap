# Places Category Discovery and List Filters Implementation Plan
...
**Goal:** Let callers find category values in one command, filter a category together with everything beneath it, and get a clear error instead of a traceback or a silent zero.
...
**Spec:** `docs/superpowers/specs/2026-09-20-places-category-discovery-and-list-filters.md`

**Builds on:** `docs/superpowers/plans/2026-09-20-places-category-taxonomy-migration.md`, which covers phases 1 and 2.

## Phase layout

Phases are numbered from 3 because phases 1 and 2 are in the migration plan.
Task numbers match the commits and PRs that refer to them.
...
| **6. Docs and evals** | 6.1 to 6.4 | Skill, help text, README, SPEC, evals and performance notes cover the new operators. |
...
End state: the public docs, generated help and eval harness cover the new operators.

The field-rename doc updates shipped with the migration: `taxonomy.primary` and
`basic_category` in the Skill, README and SPEC, the `at --category` fix, and
scoping the category flags to `places`. This phase covers what the operators add.
...
- [ ] Teach `taxonomy.hierarchy`.
- [ ] Document `contains` and `~`, and state that `~` is substring matching.
- [ ] Add `contains` to the category-field guidance for roll-up.
- [ ] Update command help text for commands with `--where`.
...
- [ ] Add examples for `--find`, `~` and `contains`.
...
- [ ] Run the eval bank before the operator changes, and save the baseline result.
- [ ] Run the eval bank after the operator changes, and save the result beside the baseline.
...
If network access is available, also run one small smoke query using `--find`, `~` and `contains` against a recent release that has `taxonomy.hierarchy`.
